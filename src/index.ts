...
import { generateUUID, generateX25519KeyPair, generateShortId, generatePassword, generateShadowsocks2022Key, isValidUUID, isValidX25519Key } from './utils/crypto.js';
  ...
  try {
    switch (name) {
      ...
      case 'generate_client_config': {
        ...
        if (input.http_port) response += `**Local HTTP Port:** ${input.http_port}\n`;
      ...
      case 'generate_short_id': {
        const length = (args as { length?: number }).length ?? 8;
      ...
      case 'generate_password': {
        const length = (args as { length?: number }).length ?? 16;
      ...
      case 'generate_ss2022_key': {
        const cipher = (args as { cipher?: string }).cipher ?? '2022-blake3-aes-256-gcm';
