 ...
 */
export function generateServerConfig(input: GenerateServerConfigInput): {
  config: SimpleXrayConfig;
  users: Array<{ id: string; email?: string }>;
  realityKeys?: { privateKey: string; publicKey: string };
} {
  ...
  let realityKeys: { privateKey: string; publicKey: string } | undefined;
  if (input.security === 'reality') {
...
function generateUsers(input: GenerateServerConfigInput): Array<{ id: string; email?: string; level?: number }> {
...
function buildInbound(
  input: GenerateServerConfigInput,
  users: Array<{ id: string; email?: string; level?: number }>,
  realityKeys?: { privateKey: string; publicKey: string }
...
function buildStreamSettings(
  ...
  if (input.security === 'tls') {
    ...
    if (input.tls_server_name) {
      (streamSettings.tlsSettings as Record<string, unknown>).serverName = input.tls_server_name;
    }
...
function buildOutbounds(): Array<Record<string, unknown>> {
  return [
    {
      tag: 'direct',
      protocol: 'freedom',
...
function buildRouting(input: GenerateServerConfigInput): Record<string, unknown> {
...
function buildDns(): Record<string, unknown> {
  return {
    servers: [
      '8.8.8.8',
      '1.1.1.1',
